"""
Estrategia CRT de Extremo H4 + FVG
Combina detección de CRT de Extremo en H4 con entradas basadas en FVG
"""
...
class CRTextremeStrategy(BaseStrategy):
    """
    Estrategia CRT de Extremo H4 + FVG
    
    Lógica:
    1. Detecta CRT de Extremo en H4 (vela 5 AM barre AMBOS extremos de vela 1 AM)
    2. Define TP según el tipo de cierre de la vela 5 AM:
    ...
    4. Busca entrada en FVG según la dirección del objetivo
    5. Ejecuta orden con RR mínimo 1:2 (puede ser mayor si TP lógico lo requiere)
    """
    ...
    def __init__(self, config: Dict):
        ...
        self.logger.info(f"CRTextremeStrategy inicializada - Entry: {self.entry_timeframe}, RR: {self.min_rr}")
    ...
    def analyze(self, symbol: str, rates: np.ndarray) -> Optional[Dict]:
        ...
        try:
            ...
            # 2. Detectar CRT de Extremo en H4
            self.logger.info(f"[{symbol}] 🔍 Etapa 2/4: Buscando CRT de Extremo en H4...")
            crt_extreme = detect_crt_extreme(symbol)
            
            if not crt_extreme or not crt_extreme.get('detected'):
                # Si estaba monitoreando, cancelar monitoreo
                if self.monitoring_fvg:
                    self.logger.info(f"[{symbol}] ⏸️  CRT de Extremo desapareció - Cancelando monitoreo intensivo")
                    self.monitoring_fvg = False
                    self.monitoring_fvg_data = None
                self.logger.info(f"[{symbol}] ⏸️  Etapa 2/4: Esperando - No hay CRT de Extremo detectado en H4")
                return None
            
            sweep_type = crt_extreme.get('sweep_type')
            direction = crt_extreme.get('direction')
            target_price = crt_extreme.get('target_price')
            swept_high = crt_extreme.get('swept_high')
            swept_low = crt_extreme.get('swept_low')
            close_type = crt_extreme.get('close_type')
            
            # Determinar tipo de CRT de forma clara
            crt_type_name = "EXTREMO"
            
            # Determinar objetivo según el tipo de cierre de la vela 5 AM
            target_extreme = None
            if direction == 'BULLISH':
                target_extreme = "HIGH de vela 5 AM (cerró alcista)"
            elif direction == 'BEARISH':
                target_extreme = "LOW de vela 5 AM (cerró bajista)"
            
            # Log estructurado y claro
            self.logger.info(f"[{symbol}] {'='*70}")
            self.logger.info(f"[{symbol}] ✅ CRT DE EXTREMO DETECTADO - Etapa 2/4 COMPLETA")
            self.logger.info(f"[{symbol}] {'='*70}")
            self.logger.info(f"[{symbol}] 📊 TIPO DE CRT: {crt_type_name}")
            self.logger.info(f"[{symbol}] 📍 Detalles del Patrón:")
            self.logger.info(f"[{symbol}]    • Barrido: Vela 5 AM barrió AMBOS extremos de vela 1 AM")
            swept_high_str = f"{swept_high:.5f}" if swept_high is not None else 'N/A'
            swept_low_str = f"{swept_low:.5f}" if swept_low is not None else 'N/A'
            self.logger.info(f"[{symbol}]    • Barrió HIGH: {swept_high_str}")
            self.logger.info(f"[{symbol}]    • Barrió LOW: {swept_low_str}")
            self.logger.info(f"[{symbol}]    • Tipo de cierre de vela 5 AM: {close_type}")
            self.logger.info(f"[{symbol}] {'-'*70}")
            self.logger.info(f"[{symbol}] 🎯 OBJETIVO (TP) SEGÚN CRT DE EXTREMO:")
            ...
            candle_9am = crt_extreme.get('candle_9am')
            if candle_9am:
                ...
                if direction == 'BULLISH' and candle_9am_high and target_price:
                    # Para Extremo Alcista: El HIGH de la vela 9 AM debe ser >= al objetivo (HIGH de vela 5 AM)
                    if candle_9am_high >= target_price:
                        objective_reached = True
                elif direction == 'BEARISH' and candle_9am_low and target_price:
                    # Para Extremo Bajista: El LOW de la vela 9 AM debe ser <= al objetivo (LOW de vela 5 AM)
                ...
                if objective_reached:
                    ...
                    if not has_open_positions:
                        ...
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 🎯 OBJETIVO DEL CRT DE EXTREMO ALCANZADO")
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 📊 Tipo de CRT: {crt_type_name}")
            ...
            if entry_signal:
                ...
                if fvg and self._is_expected_fvg(fvg, crt_extreme):
                    ...
                    if not self.monitoring_fvg:
                        ...
                        target_extreme = None
                        if crt_extreme.get('direction') == 'BULLISH':
                            target_extreme = "HIGH de vela 5 AM (cerró alcista)"
                        elif crt_extreme.get('direction') == 'BEARISH':
                            target_extreme = "LOW de vela 5 AM (cerró bajista)"
                        
                        target_price_log = crt_extreme.get('target_price')
                        ...
                        self.logger.info(f"[{symbol}] 🎯 OBJETIVO SEGÚN CRT DE EXTREMO: {target_extreme} = {target_price_str}")
                        self.logger.info(f"[{symbol}] {'-'*70}")
    ...
    def _is_expected_fvg(self, fvg: Dict, crt_extreme: Dict) -> bool:
        """
        Verifica si un FVG es el esperado según el CRT de Extremo
        
        Args:
            fvg: Información del FVG
            crt_extreme: Información del CRT de Extremo
        ...
        try:
            ...
            # Determinar qué tipo de FVG buscamos según la dirección del objetivo del CRT de Extremo
            # El FVG debe estar en la MISMA dirección del objetivo
            # - Extremo Alcista (cerró alcista → TP = HIGH de vela 5 AM) → busca FVG ALCISTA (para ir hacia arriba)
            # - Extremo Bajista (cerró bajista → TP = LOW de vela 5 AM) → busca FVG BAJISTA (para ir hacia abajo)
            expected_fvg_type = None
            if direction == 'BULLISH':
                # Extremo Alcista: objetivo es HIGH de vela 5 AM → necesitamos FVG ALCISTA para ir hacia arriba
                expected_fvg_type = 'ALCISTA'
            elif direction == 'BEARISH':
                # Extremo Bajista: objetivo es LOW de vela 5 AM → necesitamos FVG BAJISTA para ir hacia abajo
    ...
    def _monitor_fvg_intensive(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            crt_extreme = self.monitoring_fvg_data.get('crt_extreme')
            if not crt_extreme:
                self.monitoring_fvg = False
                self.monitoring_fvg_data = None
                return None
            
            # Verificar que el CRT de Extremo aún existe
            current_crt = detect_crt_extreme(symbol)
            if not current_crt or not current_crt.get('detected'):
                self.logger.info(f"[{symbol}] ⏸️  CRT de Extremo desapareció durante monitoreo - Cancelando")
                self.monitoring_fvg = False
                self.monitoring_fvg_data = None
                return None
            ...
            if candle_9am and target_price:
                ...
                if objective_reached:
                    ...
                    if not has_open_positions:
                        ...
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 🎯 OBJETIVO DEL CRT DE EXTREMO ALCANZADO (durante monitoreo)")
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 📊 Tipo de CRT: {crt_type_name}")
                        self.logger.info(f"[{symbol}] 🎯 Objetivo esperado: {target_price_str}")
                        if direction == 'BULLISH':
            ...
            fvg = detect_fvg(symbol, self.entry_timeframe)
            if not fvg or not self._is_expected_fvg(fvg, crt_extreme):
    ...
    def _find_fvg_entry(self, symbol: str, crt_extreme: Dict) -> Optional[Dict]:
        """
        Busca entrada en FVG en la MISMA dirección del objetivo del CRT de Extremo
        - Extremo Alcista (cerró alcista → TP = HIGH de vela 5 AM) → busca FVG ALCISTA (para ir hacia arriba)
        - Extremo Bajista (cerró bajista → TP = LOW de vela 5 AM) → busca FVG BAJISTA (para ir hacia abajo)
        ...
        Args:
            symbol: Símbolo
            crt_extreme: Información del CRT de Extremo detectado
        ...
        try:
            ...
            # Verificar si el FVG es el esperado según el CRT de Extremo
            if not self._is_expected_fvg(fvg, crt_extreme):
                self.logger.info(f"[{symbol}] ⏸️  FVG detectado ({fvg_type}) no es el esperado según CRT de Extremo ({sweep_type} → {direction})")
                return None
            ...
            # Verificar que el FVG formado es del tipo esperado según el CRT de Extremo
            ...
            if direction == 'BULLISH':
                ...
                # Extremo Bajista: objetivo es LOW de vela 5 AM → necesitamos FVG BAJISTA
                expected_fvg_type = 'BAJISTA'
            ...
            fvg_top = fvg.get('fvg_top')
            fvg_bottom = fvg.get('fvg_bottom')
            target_price = crt_extreme.get('target_price')  # TP viene del CRT de Extremo
            ...
            if initial_rr < self.min_rr:
                ...
                self.logger.info(
                    ...
                    f"TP ajustado: {crt_extreme.get('target_price'):.5f} → {take_profit:.5f} para cumplir RR mínimo"
                )
            else:
    ...
    def _execute_order(self, symbol: str, crt_extreme: Dict, entry_signal: Dict) -> Optional[Dict]:
        """
        ...
        try:
            ...
            # El TP viene del CRT de Extremo (HIGH o LOW de vela 5 AM según cierre)
            ...
            self.logger.info(f"[{symbol}] {'='*70}")
            self.logger.info(f"[{symbol}] 💹 EJECUTANDO ORDEN CRT DE EXTREMO")
            self.logger.info(f"[{symbol}] {'='*70}")
            self.logger.info(f"[{symbol}] 📊 TIPO DE CRT: {crt_type_name}")
            ...
            self.logger.info(f"[{symbol}]    • Barrió HIGH: {swept_high_str}")
            self.logger.info(f"[{symbol}]    • Barrió LOW: {swept_low_str}")
            self.logger.info(f"[{symbol}]    • Tipo de cierre de vela 5 AM: {crt_extreme.get('close_type', 'N/A')}")
            self.logger.info(f"[{symbol}] {'-'*70}")
            ...
            if direction == 'BULLISH':
                result = self.executor.buy(
                    ...
                    stop_loss=stop_loss,
                    take_profit=take_profit,
                    comment=f"CRT Extremo H4 + FVG {self.entry_timeframe}"
                )
            else:
