"""
Estrategia CRT de Continuación H4 + FVG
Combina detección de CRT de Continuación en H4 con entradas basadas en FVG
"""
...
class CRTContinuationStrategy(BaseStrategy):
    """
    Estrategia CRT de Continuación H4 + FVG
    
    Lógica:
    1. Detecta CRT de Continuación en H4 (vela 5 AM barre extremos de 1 AM y cierra con cuerpo fuera)
    2. Define TP basado en extremo de vela 5 AM (HIGH si continuación alcista, LOW si continuación bajista)
       - El objetivo se define desde la vela de 5 AM
       - Esperamos que el precio alcance ese objetivo durante la vela de 9 AM NY
    3. Verifica noticias de alto impacto
    4. Busca entrada en FVG según la dirección de continuación
    ...
    def __init__(self, config: Dict):
        ...
        self.logger.info(f"CRTContinuationStrategy inicializada - Entry: {self.entry_timeframe}, RR: {self.min_rr}")
    ...
    def analyze(self, symbol: str, rates: np.ndarray) -> Optional[Dict]:
        ...
        try:
            ...
            # 2. Detectar CRT de Continuación en H4
            self.logger.info(f"[{symbol}] 🔍 Etapa 2/4: Buscando CRT de Continuación en H4...")
            crt_continuation = detect_crt_continuation(symbol)
            
            if not crt_continuation or not crt_continuation.get('detected'):
                # Si estaba monitoreando, cancelar monitoreo
                if self.monitoring_fvg:
                    self.logger.info(f"[{symbol}] ⏸️  CRT de Continuación desapareció - Cancelando monitoreo intensivo")
                    self.monitoring_fvg = False
                    self.monitoring_fvg_data = None
                self.logger.info(f"[{symbol}] ⏸️  Etapa 2/4: Esperando - No hay CRT de Continuación detectado en H4")
                return None
            
            sweep_type = crt_continuation.get('sweep_type')
            direction = crt_continuation.get('direction')
            target_price = crt_continuation.get('target_price')
            sweep_price = crt_continuation.get('sweep_price')
            close_type = crt_continuation.get('close_type')
            body_outside = crt_continuation.get('body_outside')
            swept_extreme = crt_continuation.get('swept_extreme')
            ...
            self.logger.info(f"[{symbol}] ✅ CRT DE CONTINUACIÓN DETECTADO - Etapa 2/4 COMPLETA")
            self.logger.info(f"[{symbol}] {'='*70}")
            self.logger.info(f"[{symbol}] 📊 TIPO DE CRT: {crt_type_name}")
            self.logger.info(f"[{symbol}] 📍 Detalles del Patrón:")
            self.logger.info(f"[{symbol}]    • Barrido: Vela 5 AM barrió {swept_extreme.upper()} de vela 1 AM")
            ...
            self.logger.info(f"[{symbol}]    • Cierre: Cuerpo de vela 5 AM cerró {body_outside.upper()} del rango de vela 1 AM")
            self.logger.info(f"[{symbol}]    • Tipo de cierre: {close_type}")
            self.logger.info(f"[{symbol}] {'-'*70}")
            self.logger.info(f"[{symbol}] 🎯 OBJETIVO (TP) SEGÚN CRT:")
            self.logger.info(f"[{symbol}]    • Tipo: {crt_type_name}")
            self.logger.info(f"[{symbol}]    • Objetivo definido desde: {target_extreme}")
            ...
            candle_9am = crt_continuation.get('candle_9am')
            if candle_9am:
                ...
                if direction == 'BULLISH' and candle_9am_high and target_price:
                    # Para Continuación Alcista: El HIGH de la vela 9 AM debe ser >= al objetivo (HIGH de vela 5 AM)
                    if candle_9am_high >= target_price:
                        objective_reached = True
                elif direction == 'BEARISH' and candle_9am_low and target_price:
                    # Para Continuación Bajista: El LOW de la vela 9 AM debe ser <= al objetivo (LOW de vela 5 AM)
                ...
                if objective_reached:
                    ...
                    if not has_open_positions:
                        ...
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 🎯 OBJETIVO DEL CRT DE CONTINUACIÓN ALCANZADO")
                        self.logger.info(f"[{symbol}] {'='*70}")
                        self.logger.info(f"[{symbol}] 📊 Tipo de CRT: {crt_type_name}")
            ...
            if entry_signal:
                ...
                if fvg and self._is_expected_fvg(fvg, crt_continuation):
                    ...
                    if not self.monitoring_fvg:
                        ...
                        target_price_log = crt_continuation.get('target_price')
                        ...
                        self.logger.info(f"[{symbol}] 🎯 OBJETIVO SEGÚN CRT: {target_extreme} = {target_price_str}")
                        self.logger.info(f"[{symbol}] {'-'*70}")
                        ...
                        self.monitoring_fvg_data = {
                            'crt_continuation': crt_continuation,
                            'fvg': fvg
                        }
                    else:
                        # Actualizar datos del FVG si ya está monitoreando
                        self.monitoring_fvg_data['fvg'] = fvg
                        self.monitoring_fvg_data['crt_continuation'] = crt_continuation
    ...
    def _is_expected_fvg(self, fvg: Dict, crt_continuation: Dict) -> bool:
        """
        Verifica si un FVG es el esperado según el CRT de Continuación
        
        Args:
            fvg: Información del FVG
            crt_continuation: Información del CRT de Continuación
        ...
        try:
            ...
            # Determinar qué tipo de FVG buscamos según la dirección del objetivo del CRT
            # El FVG debe estar en la MISMA dirección del objetivo (no en dirección contraria)
            # - Continuación Alcista (objetivo = HIGH de vela 5 AM) → busca FVG ALCISTA (para ir hacia arriba)
            # - Continuación Bajista (objetivo = LOW de vela 5 AM) → busca FVG BAJISTA (para ir hacia abajo)
            expected_fvg_type = None
            if direction == 'BULLISH':
                # Continuación Alcista: objetivo es HIGH de vela 5 AM → necesitamos FVG ALCISTA para ir hacia arriba
                expected_fvg_type = 'ALCISTA'
            elif direction == 'BEARISH':
                # Continuación Bajista: objetivo es LOW de vela 5 AM → necesitamos FVG BAJISTA para ir hacia abajo
    ...
    def _monitor_fvg_intensive(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            crt_continuation = self.monitoring_fvg_data.get('crt_continuation')
            if not crt_continuation:
                self.monitoring_fvg = False
                self.monitoring_fvg_data = None
                return None
            
            # Verificar que el CRT de Continuación aún existe
            current_crt = detect_crt_continuation(symbol)
            if not current_crt or not current_crt.get('detected'):
                self.logger.info(f"[{symbol}] ⏸️  CRT de Continuación desapareció durante monitoreo - Cancelando")
                self.monitoring_fvg = False
                self.monitoring_fvg_data = None
                return None
            ...
            if candle_9am and target_price:
                ...
                if objective_reached:
                    ...
                    if not has_open_positions:
                        # Determinar tipo de CRT para logs
                        crt_type_name = "CONTINUACIÓN ALCISTA" if direction == 'BULLISH' else "CONTINUACIÓN BAJISTA"
            ...
            fvg = detect_fvg(symbol, self.entry_timeframe)
            if not fvg or not self._is_expected_fvg(fvg, crt_continuation):
    ...
    def _check_daily_trade_limit(self, symbol: str) -> bool:
        ...
        if db_manager.enabled:
            strategy_name = 'crt_continuation'
    ...
    def _find_fvg_entry(self, symbol: str, crt_continuation: Dict) -> Optional[Dict]:
        """
        ...
        Args:
            symbol: Símbolo
            crt_continuation: Información del CRT de Continuación detectado
        ...
        try:
            ...
            # Verificar si el FVG es el esperado según el CRT de Continuación
            if not self._is_expected_fvg(fvg, crt_continuation):
                self.logger.info(f"[{symbol}] ⏸️  FVG detectado ({fvg_type}) no es el esperado según CRT ({sweep_type} → {direction})")
                return None
            ...
            # Verificar que el FVG formado es del tipo esperado según el CRT de Continuación
            ...
            # Verificar tipo de FVG según la dirección del objetivo del CRT
            # El FVG debe estar en la MISMA dirección del objetivo
            expected_fvg_type = None
            if direction == 'BULLISH':
            ...
            fvg_top = fvg.get('fvg_top')
            fvg_bottom = fvg.get('fvg_bottom')
            target_price = crt_continuation.get('target_price')  # TP viene del CRT de Continuación
            ...
            if initial_rr < self.min_rr:
                ...
                self.logger.info(
                    ...
                    f"TP ajustado: {crt_continuation.get('target_price'):.5f} → {take_profit:.5f} para cumplir RR mínimo"
                )
            else:
    ...
    def _execute_order(self, symbol: str, crt_continuation: Dict, entry_signal: Dict) -> Optional[Dict]:
        """
        ...
        try:
            ...
            # El TP viene del CRT (extremo de vela 5 AM para Continuación)
            ...
            self.logger.info(f"[{symbol}]    • Tipo de CRT: {crt_type_name}")
            self.logger.info(f"[{symbol}]    • Barrido: Vela 5 AM barrió {crt_continuation.get('swept_extreme', 'N/A').upper()} de vela 1 AM")
            sweep_price = crt_continuation.get('sweep_price')
            sweep_price_str = f"{sweep_price:.5f}" if sweep_price is not None else 'N/A'
            self.logger.info(f"[{symbol}]    • Precio barrido: {sweep_price_str}")
            self.logger.info(f"[{symbol}]    • Cierre: Cuerpo de vela 5 AM cerró {crt_continuation.get('body_outside', 'N/A').upper()} del rango")
            self.logger.info(f"[{symbol}] {'-'*70}")
            ...
            if direction == 'BULLISH':
                result = self.executor.buy(
                    ...
                    stop_loss=stop_loss,
                    take_profit=take_profit,
                    comment=f"CRT Continuación H4 + FVG {self.entry_timeframe}"
                )
            else:
